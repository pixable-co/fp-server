...
class {$className} {
    ...
    public static function init() {
        ...
        add_action('wp_ajax_fpserver/{$ajaxName}', array(\$self, '{$ajaxName}'));
PHP;
        ...
        add_action('wp_ajax_nopriv_fpserver/{$ajaxName}', array(\$self, '{$ajaxName}'));
PHP;
        }
...
PHP;
        ...
        if (file_exists($ajaxClassFilePath)) {
            ...
            $useStatement = "use FPServer\\{$className};";
            if (strpos($ajaxContent, $useStatement) === false) {
                $ajaxContent = str_replace("namespace FPServer;\n", "namespace FPServer;\n\n{$useStatement}\n", $ajaxContent);
            }
